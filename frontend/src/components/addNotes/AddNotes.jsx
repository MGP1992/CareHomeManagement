...
const AddNotes = (props) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:8082/residents/add-note', data)
      console.log('this is the response after the database has been pinged', res);
      setResident({
        ...resident,
        notes: {
          activities: [],
  ...
  return (
    ...
    <Modal title="New Note" className="notes-modal" onClose={() => setShow(false)} show={show}>
      ...
      <select id="dropdown" name="category" value={category} onChange={onChange}>
